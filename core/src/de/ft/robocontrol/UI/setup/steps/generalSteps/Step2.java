...
public class Step2 {
    ...
    public static void step2(final VisTable builder) {
        ...
        if(SetupWindow.tempverbindungsspeicher.device== Devices.RASPBERRYPI4){
            selectPlatform.setSelected("Raspberry Pi");
            update1();
            selectBoardArt.setSelected("Raspberry Pi 4b");
            update2();
        }
        if(SetupWindow.tempverbindungsspeicher.device==Devices.RASPBERRY3Bplus){
            selectPlatform.setSelected("Raspberry Pi");
            update1();
            selectBoardArt.setSelected("Raspberry pi 3b/3b+");
            update2();
        }
        if(SetupWindow.tempverbindungsspeicher.device==Devices.RASPBERRYPIZEROW){
            selectPlatform.setSelected("Raspberry Pi");
            update1();
            selectBoardArt.setSelected("Raspberry Pi Zero W");
            update2();
        }
        if(SetupWindow.tempverbindungsspeicher.device==Devices.EV3){
            selectPlatform.setSelected("EV3");
            update1();
            selectBoardArt.setSelected("Normal");
            update2();
        }
        if(SetupWindow.tempverbindungsspeicher.device==Devices.EV3_DEV){
            selectPlatform.setSelected("EV3");
            update1();
