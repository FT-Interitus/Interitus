...
class CheckBox : UIElement {
    ...
    override fun draw() {
        ...
        if (!disabled && WindowAPI.isButtonPressed(0) && CheckCollision.checkpointwithobject(
                x,
                y,
                size,
                size,
                Var.mouseDownPosWithoutUnproject.x,
                WindowAPI.getHeight() - Var.mouseDownPosWithoutUnproject.y
            )
        ) {
            xMove = movement
            yMove = movement
        ...
        if (clickedOn && !disabled && !WindowAPI.isButtonPressed(0) && CheckCollision.checkpointwithobject(
                x,
                y,
                size,
                size,
                Var.mouseReleasePosWithoutUnproject.x,
                WindowAPI.getHeight() - Var.mouseReleasePosWithoutUnproject.y
            )
        ) {
            clickedOn = false
        ...
        if (toggleState) {
            ...
            WindowManager.shapeRenderer.rect(
                x.toFloat() + xMove + InnerBoarderMargin,
                y.toFloat() - yMove + InnerBoarderMargin,
                size.toFloat() - InnerBoarderMargin * 2,
                size.toFloat() - InnerBoarderMargin * 2
            )
        }
