...
public class subitem13 {
   ...
   private static final ArrayList<Integer> shownpluginids = new ArrayList<>();

    /***
     * Builds the Plugin Store into the VisTable
     * @param builder
     */
    public static void add(final VisTable builder) {
        ...
        shownpluginids.add(StorePluginsVar.pluginEntries.get(i).getId());
            ...
            visTables.get(i).addListener(new ClickListener() {
                ...
                public void clicked(InputEvent event, float x, float y) {
                   PluginPage.add(builder,StorePluginsVar.pluginEntries.get(visTables.indexOf(event.getListenerActor())));
                }
            });
            ...
            public void run() {
                ...
                try {
                    ...
                    if (scrollPane.getScrollPercentY() > 0.3f && AssetLoader.storeimages.size() < StorePluginsVar.pluginEntries.size()) { //Wenn weiter als 50% gescrollt ist und es neue Plugins gibt
                        ...
                        for (int i=0; visTables.size() != StorePluginsVar.pluginEntries.size();i++ ) {
                            byte[] download = null;
                            try {
                            ...
                            visTables.get(visTables.size()-1).add(new VisLabel(StorePluginsVar.pluginEntries.get(visTables.size() - 1).getName())).padLeft(-100).padRight(0).padTop(-60).padBottom(20);
                            visTables.get(visTables.size()-1).add(new VisLabel(StorePluginsVar.pluginEntries.get(visTables.size() - 1).getDescription())).padLeft(-100).padRight(-200).padTop(-15).align(Align.right);
            ...
            public void changed(ChangeEvent event, Actor actor) {
                ...
                if (search.getText() != "" && search.getText() != " " && search.getText().length() > 0&&pluginimage.size()==StorePluginsVar.pluginEntries.size()) {
                    ...
                    for (int i = 0; i < result.size(); i++) {
                        ...
                        visTables.add(new VisTable());
                        shownpluginids.add(StorePluginsVar.pluginEntries.get(StorePluginsVar.pluginEntries.indexOf(result.get(i))).getId());
     ...
     * Builds the Plugin Store into the VisTable
     * @param builder
     * @param percent give the position of the scroll pane
     * @see PluginPage
     */
    public static void add(final VisTable builder,final float percent) {
