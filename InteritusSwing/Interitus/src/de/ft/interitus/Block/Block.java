...
public abstract class Block implements VisibleObjects {
    ...
    public boolean isVisible() {
        ...
        //TODO L return Viewport.extendedfrustum.boundsInFrustum(this.getX(), this.getY(), 0, this.getW(), this.getH(), 0); //Ist der Block im Camera bereich?
        return true;
    ...
    public void draw(Graphics2D g2d) {
        ...
        if (ProjectManager.getActProjectVar().biggestblock == this) {
            if (this.isShowdupulicate_rechts() && this.getBlocktype().canhasrightconnector()) {
                //TODO L batch.setColor(1, 1, 1, 0.5f);
                //TODO L batch.draw(AssetLoader.block_middle, this.getX_dup_rechts(), this.y, ProjectManager.getActProjectVar().markedblock.getW(), this.getH()); //Wenn der Block die größte überlappung hat wird er als show duplicat angezigt
                //TODO L batch.setColor(1, 1, 1, 1);
            }

            if (this.isShowdupulicate_links() && this.getBlocktype().canhasleftconnector()&&ProjectManager.getActProjectVar().markedblock!=null) {
                //TODO L batch.setColor(1, 1, 1, 0.5f);
                //TODO L batch.draw(AssetLoader.block_middle, this.x - ProjectManager.getActProjectVar().markedblock.getW(), this.y, ProjectManager.getActProjectVar().markedblock.getW(), this.getH()); //das gleiche für links
                //TODO L batch.setColor(1, 1, 1, 1);
            }
        }
        ...
        try {
            if (ProjectManager.getActProjectVar().Blockwitherrors.contains(this.getIndex())) {
                //TODO L batch.setColor(1, 0.8f, 0.8f, 1);
            } else {
                //TODO L batch.setColor(1, 1, 1, 1);
            }
            ...
            if (!Settings.disableblockgrayout) {
                if (!ProjectManager.getActProjectVar().projectType.getProjectFunktions().isblockconnected(this)) {
                    //TODO L batch.setColor(batch.getColor().r, batch.getColor().g, batch.getColor().b, 0.4f);
                }
            }
            //TODO L batch.draw(AssetLoader.block_middle, this.getX() + 6, this.getY(), this.getW() - 12, this.getH()); // Block ohne das er makiert ist
            //TODO L batch.draw(AssetLoader.block_left, this.getX(), this.getY(), 6, this.getH());
            //TODO L batch.draw(AssetLoader.block_right, this.getX() + this.getW() - 6, this.getY(), 6, this.getH());
            ...
            if (blocktype.getBlockCategorie() != null) {
                switch (blocktype.getBlockCategorie()) {
                    ...
                    case ActionBlocks -> {
                        ...
                        //TODO L batch.draw(AssetLoader.green_bar_middle, this.getX() + 6, this.getY() - 1 + this.getH() - 13, this.getW() - 12, 13); // Block ohne das er makiert ist
                        //TODO L batch.draw(AssetLoader.green_bar_left, this.getX() + 1, this.getY() - 1 + this.getH() - 13, 6, 13);
                        //TODO L batch.draw(AssetLoader.green_bar_right, this.getX() + this.getW() - 7, this.getY() - 1 + this.getH() - 13, 6, 13);
                    ...
                    case Programm_Sequence -> {
                        //TODO L batch.draw(AssetLoader.orange_bar_middle, this.getX() + 6, this.getY() - 1 + this.getH() - 13, this.getW() - 12, 13); // Block ohne das er makiert ist
                        //TODO L batch.draw(AssetLoader.orange_bar_left, this.getX() + 1, this.getY() - 1 + this.getH() - 13, 6, 13);
                        //TODO L batch.draw(AssetLoader.orange_bar_right, this.getX() + this.getW() - 7, this.getY() - 1 + this.getH() - 13, 6, 13);

                    }

                    case Sensors -> {
                        //TODO L batch.draw(AssetLoader.yellow_bar_middle, this.getX() + 6, this.getY() - 1 + this.getH() - 13, this.getW() - 12, 13); // Block ohne das er makiert ist
                        //TODO L batch.draw(AssetLoader.yellow_bar_left, this.getX() + 1, this.getY() - 1 + this.getH() - 13, 6, 13);
                        //TODO L batch.draw(AssetLoader.yellow_bar_right, this.getX() + this.getW() - 7, this.getY() - 1 + this.getH() - 13, 6, 13);

                    }

                    case Data_Operation -> {
                        //TODO L batch.draw(AssetLoader.red_bar_middle, this.getX() + 6, this.getY() - 1 + this.getH() - 13, this.getW() - 12, 13); // Block ohne das er makiert ist
                        //TODO L batch.draw(AssetLoader.red_bar_left, this.getX() + 1, this.getY() - 1 + this.getH() - 13, 6, 13);
                        //TODO L batch.draw(AssetLoader.red_bar_right, this.getX() + this.getW() - 7, this.getY() - 1 + this.getH() - 13, 6, 13);
                    ...
                    case Specials -> {
                        //TODO L batch.draw(AssetLoader.blue_bar_middle, this.getX() + 6, this.getY() - 1 + this.getH() - 13, this.getW() - 12, 13); // Block ohne das er makiert ist
                        //TODO L batch.draw(AssetLoader.blue_bar_left, this.getX() + 1, this.getY() - 1 + this.getH() - 13, 6, 13);
                        //TODO L batch.draw(AssetLoader.blue_bar_right, this.getX() + this.getW() - 7, this.getY() - 1 + this.getH() - 13, 6, 13);

                    }

                    case OwnBlocks -> {
                        //TODO L batch.draw(AssetLoader.turquoise_bar_middle, this.getX() + 6, this.getY() - 1 + this.getH() - 13, this.getW() - 12, 13); // Block ohne das er makiert ist
                        //TODO L batch.draw(AssetLoader.turquoise_bar_left, this.getX() + 1, this.getY() - 1 + this.getH() - 13, 6, 13);
                        //TODO L batch.draw(AssetLoader.turquoise_bar_right, this.getX() + this.getW() - 7, this.getY() - 1 + this.getH() - 13, 6, 13);
            ...
            if (this.blockupdate.toggle) {
                ...
                //TODO L batch.draw(AssetLoader.mouse_over_mitte, this.getX() + 6, this.getY(), this.getW() - 12, this.getH()); // Block ohne das er makiert ist
                //TODO L batch.draw(AssetLoader.mouseover_links, this.getX(), this.getY(), 6, this.getH());
                //TODO L batch.draw(AssetLoader.mouse_over_rechts, this.getX() + this.getW() - 6, this.getY(), 6, this.getH());
            }

            if (this.isMarked()) {

                //TODO L batch.draw(AssetLoader.marked_mitte, this.getX() + 6, this.getY(), this.getW() - 12, this.getH()); // Block ohne das er makiert ist
                //TODO L batch.draw(AssetLoader.marked_links, this.getX(), this.getY(), 6, this.getH());
                //TODO L batch.draw(AssetLoader.marked_rechts, this.getX() + this.getW() - 6, this.getY(), 6, this.getH());
            }
 ...
 */
            ...
            if (!this.blockupdate.isIsconnectorclicked() && ProjectManager.getActProjectVar().showleftdocker && this.getLeft() == null && this.getBlocktype().canhasleftconnector()) {
                //TODO L batch.draw(AssetLoader.connector_offerd, getWireconnector_left().x, getWireconnector_left().y, 20, 20);
            }

            if (this.getRight() == null && this.getBlocktype().canhasrightconnector()) {
                //TODO L batch.draw(AssetLoader.connector, getwireconnector_right().x, getwireconnector_right().y, 20, 20);
            }
            ...
            if (this.getBlocktype().getBlockParameter() != null && this.getBlocktype().getBlockParameter().size() > 0) {
                ...
                aktualX += 5;
                //TODO L batch.draw(this.getBlocktype().getDescriptionImage(), aktualX, this.getY() + this.getH() - 30 - 17, 30, 30);

                aktualX += 35;
                for (int i = 0; i < this.getBlocktype().getBlockParameter().size(); i++) {
                    //TODO L batch.draw(this.getBlocktype().getBlockParameter().get(i).getParameterTexture(), aktualX + 5, this.getY() + 33, 20, 20);
                    if (this.getBlocktype().getBlockParameter().get(i).getParameterType().isOutput()) {
                        //TODO L batch.draw(this.getBlocktype().getBlockParameter().get(i).getParameterType().getTyp().getTextureconnector(), (int) aktualX, this.getY() - 7, UIVar.parameter_width, UIVar.parameter_height, 0, 0, AssetLoader.Plug_IntParameter.getWidth(), AssetLoader.Plug_IntParameter.getHeight(), false, true);
                        this.getBlocktype().getBlockParameter().get(i).setX((int) aktualX);
                        this.getBlocktype().getBlockParameter().get(i).setY(this.getY() - 7);

                    } else {
                        //TODO L batch.draw(this.getBlocktype().getBlockParameter().get(i).getParameterType().getTyp().getTextureconnector(), aktualX, this.getY(), UIVar.parameter_width, UIVar.parameter_height);
                        this.getBlocktype().getBlockParameter().get(i).setX((int) aktualX);
                        this.getBlocktype().getBlockParameter().get(i).setY(this.getY());
                        //TODO L font.getData().setScale(0.9f);
                        //TODO L glyphLayout.setText(font, "" + this.getBlocktype().getBlockParameter().get(i).getParameter());
                        if (this.getBlocktype().getBlockParameter().get(i).getDatawire().size() < 1) {
                            //TODO L font.draw(batch, glyphLayout, aktualX + 15 - glyphLayout.width / 2, y + glyphLayout.height * 1.5f);
                        }
                    }
                    aktualX += 30;
                }
                ...
                //TODO L batch.draw(this.getBlocktype().getDescriptionImage(), this.getX()+((this.getW()-50f)/2f), this.getY() + this.getH()  - 17-50, 50, 50);
