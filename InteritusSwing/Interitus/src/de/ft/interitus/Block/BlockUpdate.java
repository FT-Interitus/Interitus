            ...
            public void run() {
                ...
                if (!UIVar.isdialogeopend) {
                    ...
                    try {
                        ...
                        if ((isconnectorclicked && Input.isKeyPressed(KeyEvent.VK_ESCAPE))) { //Um vorzeitige wire wieder aufzulösen und ggf zu richtigen umzuwandeln
                        ...
                        if (!isconnectorclicked && ProjectManager.getActProjectVar().showleftdocker) { //Wenn der eigene Wire connector nicht ausgelöst ist aber ein anderer
                            if (CheckKollision.checkpointwithobject((int) block.getWireconnector_left().x, (int) block.getWireconnector_left().y, 20, 20, (int) ProgrammingSpace.blockCamera.unproject_x(Input.getX()), (int) ProgrammingSpace.blockCamera.unproject_y(Input.getY()))) { //Wenn die Maus über meinen connection offerer fährt...
                        ...
                        if (CheckKollision.checkpointwithobject((int) block.getwireconnector_right().x, (int) block.getwireconnector_right().y, 20, 20, (int) Var.mousepressedold.x, (int) Var.mousepressedold.y) && Input.isButtonPressed(0) && ProjectManager.getActProjectVar().movingwires == null && !IsMousealreadypressed && block.getWire_right() == null) {
                        ...
                        if (IsMousealreadypressed && !Input.isButtonPressed(0)) {
                            IsMousealreadypressed = false;
                        }

                        if (CheckKollision.checkmousewithblock(block, Var.mousepressedold) && Input.isButtonPressed(0) && ProjectManager.getActProjectVar().ismoving == false && !block.isMarked() && !ProjectManager.getActProjectVar().marked && ProjectManager.getActProjectVar().markedblock == null) {
                            ...
                            if (!CheckKollision.checkpointwithobject((int) block.getwireconnector_right().x, (int) block.getwireconnector_right().y, 20, 20, (int) Var.mousepressedold.x, (int) Var.mousepressedold.y)) {
                                ...
                                ProjectManager.getActProjectVar().unterschiedsave.set(ProgrammingSpace.blockCamera.unproject_x(Input.getX()) - block.getX(), ProgrammingSpace.blockCamera.unproject_y(Input.getY()) - block.getY());

                            }
                        }


                        if (ProjectManager.getActProjectVar().ismoving == false && !block.isMoving() && block.isMarked() && Input.isButtonPressed(0) && CheckKollision.checkmousewithblock(block)) {
                            ...
                            if (!CheckKollision.checkpointwithobject((int) block.getwireconnector_right().x, (int) block.getwireconnector_right().y, 20, 20, (int) Var.mousepressedold.x, (int) Var.mousepressedold.y)) {
                                ...
                                int feld = 2;
                                if (Math.abs(Var.mousepressedold.x - ProgrammingSpace.blockCamera.unproject_x(Input.getX())) > feld || Math.abs(Var.mousepressedold.y - ProgrammingSpace.blockCamera.unproject_y(Input.getY())) > feld) {
...
/////////////////////////////Datawire erzeigen
                        if(block.getBlocktype()!=null&&block.getBlocktype().getBlockParameter()!=null) {
                            for (int i = 0; i < block.getBlocktype().getBlockParameter().size(); i++) {
                                if (block.getBlocktype().getBlockParameter().get(i).getParameterType().isOutput() ) {
                                    ...
                                    if(CheckKollision.checkpointwithobject(block.getBlocktype().getBlockParameter().get(i).getX(),block.getBlocktype().getBlockParameter().get(i).getY(),UIVar.parameter_width,UIVar.parameter_height, Unproject.unproject())&&Input.isButtonJustPressed(0)&&ProjectManager.getActProjectVar().moveingdatawire==null){
                        ...
                        if (block.isMoving() && Input.isButtonJustPressed(0)) {
                            ...
                            if (changewillbedeleted) {
                                ...
                                if (willbedelete) {
                                    //TODO L Gdx.graphics.setCursor(Gdx.graphics.newCursor(AssetLoader.backcursor, 0, 0));
                                    ProjectManager.getActProjectVar().removeblock = true;
                                } else {
                                    //TODO L Gdx.graphics.setSystemCursor(Cursor.SystemCursor.Arrow);
                            ...
                            block.setX((int) (ProgrammingSpace.blockCamera.unproject_x(Input.getX()) - ProjectManager.getActProjectVar().unterschiedsave.x));
                            block.setY((int) (ProgrammingSpace.blockCamera.unproject_y(Input.getY()) - ProjectManager.getActProjectVar().unterschiedsave.y));
                        ...
                        if (block.seted == false && ProjectManager.getActProjectVar().biggestblock == block && !Input.isButtonPressed(0)) {
