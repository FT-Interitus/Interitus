...
public class PopupMenue {
    ...
    //TODO L    private final Texture popupButtonimage = new Texture("popupbuttonimage.png");
    int ispressed;
    ...
    private void rechtsKlickControlle() {
        ...
        if (Input.isButtonJustPressed(0) && !CheckMouse.isMouseover(this.x + ausgleichX, this.y - (buttonheight * buttons.size()), WIDTH, buttonheight * buttons.size(), false)) {
            show = false;
        }
    }
    ...
    public void draw() {
        if (show) {
            ...
            if((this.x +WIDTH)+DISTANCE>Input.getWidth()) {
            ...
            for (int i = buttons.size() - 1; i >= 0; i--) {
                ...
                //TODO L  buttons.get(i).setImage(popupButtonimage);
                buttons.get(i).draw();
            }
